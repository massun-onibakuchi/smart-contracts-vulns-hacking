...
describe("Lack of chainID validation allows signatures to be re-used across forks", async function () {
    ...
    beforeEach(async function () {
        ...
        this.signature = await signPermitApproval(name, '1', chainId, vulnToken.address, this.approve, this.nonce, this.deadline)
    });
    ...
    describe("Lack of chainId validation", async function () {
        it("should successfully permit", async function () {
            ...
            // Approve it
            await expect(vulnToken.permit(this.approve.owner, this.approve.spender, this.approve.value, this.deadline, v, r, s)).to.emit(vulnToken, "Approval")
